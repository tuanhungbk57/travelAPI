...
namespace NTH.TravelAPI.Controllers.Website
{
    ...
    public class TripsController : ControllerBase
    {
        ...
        public TripsController( DapperContext dapper)
        {
            _dapper = dapper;
        }
        // GET: api/<TripsController>
        [HttpGet("{desstr}/des/{lang}/lang")]
        public async Task<IEnumerable<object>> GetTripsByLang(string desstr, string lang)
        {
            var procedureName = "Proc_Trip_GetListByDesAndLang";
            //var parameters = new DynamicParameters();
            //parameters.Add("Id", id, DbType.Int32, ParameterDirection.Input);
            using (var connection = _dapper.CreateConnection())
            {
                var des = await connection.QueryAsync<object>
                    (procedureName, new { v_lang = lang, v_des = desstr }, commandType: CommandType.StoredProcedure);
                return des;
            }
        ...
        {
            var procedureName = "Proc_Trip_GetByPathAndLang";
            using (var connection = _dapper.CreateConnection())
            {
